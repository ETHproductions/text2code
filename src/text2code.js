	...
	xhr.onreadystatechange = function() {
		try {
			if(xhr.readyState === XMLHttpRequest.DONE) {
				if(xhr.status !== 200)
					alert("Something went wrong while retrieving " + url + ".");
				else
					try {
						success(xhr.responseText);
					} catch (e) {
						alert("An error occured while running success(): " + e);
					}
			}
		} catch (e) {
			alert("An error occured while retrieving " + url + ": " + e);
		}
	};
	xhr.open("GET", url);
	xhr.send();
...
function text2code (text, from, to, debug, run) {
	...
	var textURL = "langs/text/"+from+".js";
	var codeURL = "langs/code/"+to+".js";
	ajax(textURL, function(response1){
	delog("Loaded text data for " + from + ".");
	eval(response1);
	ajax(codeURL, function(response2){
